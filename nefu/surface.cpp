...
int main (int argc, char** argv)
{
 ...
 if(pcl::io::loadPCDFile<pcl::PointXYZ>(argv[1],*cloud)==-1)//打开点云文件
 {
  ...
  //设置参数
  gp3.setSearchRadius (0.04);  // 设置连接点之间的最大距离（最大边长）用于确定k近邻的球半径（默认为0）
  gp3.setMu (2.5);  // 设置最近邻距离的乘子，已得到每个点的最终搜索半径（默认为0）
  gp3.setMaximumNearestNeighbors (100);  //设置搜索的最近邻点的最大数量
  gp3.setMaximumSurfaceAngle(M_PI/4); // 45 degrees 最大平面角
  gp3.setMinimumAngle(M_PI/18); // 10度 degrees 每个三角的最小角度
  gp3.setMaximumAngle(2*M_PI/3); // 120度 degrees每个三角的最大角度
