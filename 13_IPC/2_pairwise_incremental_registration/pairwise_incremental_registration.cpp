...
void loadData (int argc, char **argv, std::vector<PCD, Eigen::aligned_allocator<PCD> > &models)
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    if(fname.compare (fname.size () - extension.size (), extension.size (), extension) == 0)
    {
      ...
      models.push_back (m); //放入models中，这是个存放点云的数组
    }
...
void pairAlign (const PointCloud::Ptr cloud_src, const PointCloud::Ptr cloud_tgt, PointCloud::Ptr output, Eigen::Matrix4f &final_transform, bool downsample = false)
{
  ...
  if (downsample)
  {
    ...
    grid.setInputCloud (cloud_src); //对第一个点云采样
    grid.filter (*src);

    grid.setInputCloud (cloud_tgt);//对第二个点云采样
    grid.filter (*tgt);
  }
  else
  {
  ...
  norm_est.compute (*points_with_normals_src);//估计发现
...
//    inline void setEuclideanFitnessEpsilon (doubleepsilon) 第3个约束，前后两次迭代误差的差值。
  ...
  reg.setMaximumIterations (2); //迭代次数
    //设置匹配的点云
  ...
  for (int i = 0; i < 30; ++i)
  {
    ...
    reg.align (*reg_result); //迭代
    //在每一个迭代之间累积转换 ;getFinalTransformation获取最终的转换矩阵
